<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <p class="text-sm font-semibold uppercase tracking-[0.18em] text-slate-500">Admin workflow controls</p>
                <h1 class="mt-2 text-2xl font-semibold text-slate-900">Learner Movement Exceptions</h1>
                <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                    Review and correct transfer-out or dropout exceptions for {{ $section->gradeLevel->name }} · {{ $section->name }} without bypassing auditability or effective-date grading safeguards.
                </p>
            </div>

            <a href="{{ route('admin.submission-monitoring', ['school_year_id' => $section->school_year_id, 'section_id' => $section->id]) }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-400 hover:text-slate-900">
                Back to monitoring
            </a>
        </div>
    </x-slot>
    ...
    <div class="space-y-6">
        ...
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div class="content-card">
                ...
                <p class="mt-2 text-sm text-slate-600">Official roster entries in this section and school year.</p>
            </div>
            <div class="content-card">
                ...
                <p class="mt-2 text-sm text-slate-600">Learners currently counted in normal grading requirements.</p>
            </div>
            <div class="content-card">
        ...
        <section class="content-card">
            <form method="GET" action="{{ route('admin.submission-monitoring.sections.learner-movements.index', ['section' => $section]) }}" class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_260px_auto]">
                <div>
                ...
                <div class="flex items-end gap-3">
                    ...
                    <a href="{{ route('admin.submission-monitoring.sections.learner-movements.index', ['section' => $section]) }}" class="inline-flex items-center rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-400 hover:text-slate-900">
                        Reset
                    </a>
                </div>
            </form>
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr class="text-left text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                            ...
                            <th class="px-4 py-3">Admin correction</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200 bg-white align-top text-sm text-slate-700">
                        @forelse ($sectionRosters as $row)
                            <tr>
                                ...
                                <td class="px-4 py-4">
                                    <form method="POST" action="{{ route('admin.submission-monitoring.sections.learner-movements.update', ['section' => $section, 'section_roster' => $row['section_roster_id']]) }}" class="space-y-3">
                                        @csrf
                                        @method('PUT')
